...
export class TestViewDragAndDrop implements vscode.TreeDataProvider<Node>, vscode.TreeDragAndDropController<Node> {
	...
	public dropMimeTypes = [ 'application/vnd.code.tree.testViewDragAndDrop' ];
	...
	public _getLocalRoots(nodes: Node[]): Node[] {
		const localRoots = [];
	...
	public _reparentNode(node: Node, target: Node | undefined): void {
		const element: any = {};
	...
	public _removeNode(element: Node, tree?: any): void {
	...
	public _getChildren(key: string | undefined): string[] {
		if (!key)
	...
	public _getTreeItem(key: string): vscode.TreeItem {
		const treeElement = this._getTreeElement(key);
	...
	public _getTreeElement(element: string | undefined, tree?: any): any {
		if (!element)
			return this.tree;
	...
	public _getParent(element: string, parent?: string, tree?: any): any {
		const currentNode = tree ?? this.tree;
		for (const prop in currentNode) {
	...
	public _getNode(key: string): Node {
		if (!this.nodes[key])
